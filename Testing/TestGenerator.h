...
Test<T> GenerateTest(const size_t& array_size, const size_t& number_of_query) {
    ...
    for (auto& element : array) {
        element = static_cast<T>(rnd() % std::numeric_limits<T>::max());
    }
    ...
    for (auto& element : query) {
        ...
        if (element.type_ == false) {
            ...
            element.value_ = static_cast<T>(rnd() % std::numeric_limits<T>::max());
        } else {
            ...
            for (size_t i = 0; i < 2; i++) {
                ...
                element.right_border_ = std::max(element.right_border_border_, static_cast<size_t>(rnd() % array_size));
            }
            if (element.left_border_ > element.right_border_) {
                swap(element.left_border_, element.right_border_);
            }
        }
    }

    return Test<T>(array_size, number_of_query, array, query);
}